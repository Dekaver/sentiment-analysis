...
def retrieveProcessedData(Pdata_url):
    ...
    for i in range(len(tweets_data)):
        if str(tweets_data[i]['id']) == str(sent.iloc[i]["id"]):
            x.append(tweets_data[i]['tweet'])
...
#     test_try= vectorizer.transform(["Dont look down at people with anxiety rather give love and respect to all. shout! Equality."])
...
def datree():
    ...
    plot_confusion_matrix(nb_matrix, classes=[-1,0,1], title='Confusion matrix For datree classifier')
...
def runall():     
    retrieveTweet('data/vaksinTWEET.json')  
    retrieveProcessedData('processed_data/outputBaru1.xlsx')
    nbTrain()
    datree()
    Tsvm()
    knN()
